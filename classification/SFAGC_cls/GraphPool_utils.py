...
class GraphPool(nn.Module):
    def __init__(self,npoint,nsample,in_channel,b_c1,l_mlp_list,NL=True):
        ...
        self.bconv_0 = nn.Conv2d(in_channel,1,kernel_size=1,bias=False)
        self.bbn_0 = nn.BatchNorm2d(1)
        self.bvconv_0 = nn.Conv2d(in_channel,in_channel,kernel_size=1,bias=False)
        self.bvbn_0 = nn.BatchNorm2d(in_channel)
        self.bww_0 = nn.Conv2d(in_channel+1,1,kernel_size=1,bias=False)
        self.bwbn_0 = nn.BatchNorm2d(1)
        self.wga01_0 = nn.Conv1d(in_channel+2+in_channel,in_channel,kernel_size=1,bias=False)
        self.bnwga01_0 = nn.BatchNorm1d(in_channel)
        ...
        self.sk_conv = nn.Conv1d(in_channel*2*2,l_mlp_list[-1],1,bias=False)
    ...
    def forward(self,xyz,points,cidx=None):
        ...
        """structure_o"""
        group_idx_s = query_ball_point(K,xyz,xyz)
        ...
        cos = (inn_product / torch.sqrt((distent1 * distent2) + 1e-10)).view(B,N,K-1,1)
        ...
        new_feature = index_points(points,c_idx)#[B,S,D]
        ...
        new_group_feature = torch.cat([grouped_xyz,index_points(sf,group_idx)],dim=-1)#[B,S,K,C]
        ...
        KVl = self.lkv_conv(grouped_feature.permute(0,3,1,2))#[B,2*C,S,K]
